"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CandidateService = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("mongoose");
const candidate_details_1 = require("./model/candidate.details");
const mongoose_2 = require("@nestjs/mongoose");
const datetime_service_1 = require("../../services/datetime.service");
let CandidateService = class CandidateService {
    constructor(candidateModel, dateTimeServie) {
        this.candidateModel = candidateModel;
        this.dateTimeServie = dateTimeServie;
    }
    async candidateDetails(candidateDetailsDto, userId) {
        const candidatedetails = new this.candidateModel(candidateDetailsDto);
        candidatedetails.candidateId = userId;
        const savedUser = await candidatedetails.save();
        console.log(savedUser.id);
        return savedUser;
    }
    async getTokenId() {
        const currentDateTime = this.dateTimeServie.getCurrentDateTime();
        console.log(`*******${currentDateTime}*******`);
        return currentDateTime;
    }
    async getCandidateDetails(candidateId) {
        return this.candidateModel.findOne({ candidateId }).exec();
    }
    async updateCandidate(candidateId, updateCandidate) {
        const candidate = await this.candidateModel.findOne({ candidateId }).exec();
        if (!candidate) {
            throw new common_1.NotFoundException("Candidate Not Found");
        }
        Object.assign(candidate, updateCandidate);
        await candidate.save();
        return candidate;
    }
    async getAllCandidateDetails() {
        const candidateDetails = await this.candidateModel.find().exec();
        return candidateDetails;
    }
    async findCandidateByEmail(email) {
        const candidateDetails = await this.candidateModel.findOne({ email }).exec();
        return candidateDetails;
    }
};
exports.CandidateService = CandidateService;
exports.CandidateService = CandidateService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, mongoose_2.InjectModel)(candidate_details_1.CandidateDetails.name)),
    __metadata("design:paramtypes", [mongoose_1.Model,
        datetime_service_1.DateTimeService])
], CandidateService);
//# sourceMappingURL=candidate.service.js.map